...
var budgetedWalks = []budgetedWalk{
	...
	{fn: "collectNamedTypes", file: "schema_node.go", class: schemaDAG,
		...
		reachingPaths: "two: the table created in Root() for the name-ref stamping, and the expander's table in ExpandReferences"},
	{fn: "nodeFromJSON", file: "schema_node.go", class: textTree,
		factors:       "JSON tree nodes, in step with the compiled node at each position",
		binds:         "the parsed JSON is a TREE (each occurrence is its own text) that parse already bounded by its bracket-depth pre-scan; the compiled twin is read, never walked on its own",
		reachingPaths: "one: per Root(), over s.full with s.node alongside"},
	{fn: "nodeFromJSONObject", file: "schema_node.go", class: textTree,
		factors:       "JSON object nodes, plus the flat-field lift's rebuilt map",
		binds:         "the same JSON tree as nodeFromJSON; a flat field recurses once into the map the lift built from its own keys, so the lift adds no fan-out",
		reachingPaths: "one: nodeFromJSON's object arm, per Root()"},
	...
	{fn: "metadataDefault", file: "schema_node.go", class: valueTree,
		factors:       "default value nodes",
		binds:         "value-guided recursion over the concrete default the parse already validated; a union re-asks the wire's own branch selection, which validates a copy per branch and is what the parse paid",
		reachingPaths: "one: per Root() field default, following the value"},
